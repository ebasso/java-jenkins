    ...
    stages {
        ...
        stage('Compile') {
            steps {
                ...
                sh '/opt/IBM/ibm-java-x86_64-80/bin -d target -cp "/opt/IBM/SMP_7613/maximo/applications/maximo/lib/*.jar" @sources.txt' // Compile all Java files listed in sources.txt using the specified JDK and include JAR files from specified directories
            }
        }
